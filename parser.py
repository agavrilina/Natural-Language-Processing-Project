...
import os
scripts=os.listdir('/home/a/SNLP/imsdb')

try: 
    os.makedirs('/home/a/SNLP/database_new')
except OSError:
    if not os.path.isdir('/home/a/SNLP/database_new'):
        raise

for cntr,script in enumerate(scripts):
    print "Parsing Script %s : %d/%d" % (script.strip('.txt'), cntr+1, len(scripts))
    processed_scenes, flag = parse_data('/home/a/SNLP/imsdb/%s' % script)
    if flag >= 0.75:
        with open('/home/a/SNLP/database_new/%s.json' % script.strip('.txt'), 'w') as fp:        
